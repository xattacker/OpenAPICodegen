...
public class KotlinClientCodegen extends AbstractKotlinCodegen {
    ...
    public void processOpts() {
        ...
        // modified by xattacker on 20181016, try to load all files from folder
       // loadSupportingFile("infrastructure");
        ...
        supportingFiles.add(new SupportingFile("infrastructure/BigDecimalJsonAdapter.kt.mustache", infrastructureFolder, "BigDecimalJsonAdapter.kt"));
    }
    
    private void loadSupportingFile(String subFolder)
    {
    	try
    	{
          String infrastructureFolder = (sourceFolder + File.separator + packageName + File.separator + subFolder).replace(".", "/");
        
    	  ClassLoader loader = getClass().getClassLoader();
    	  
          URL url = loader.getResource(this.artifactId + "/" + subFolder);
          ...
          for (File f : files)
          {
        	  ...
        	  supportingFiles.add(new SupportingFile(subFolder + "/" + name, infrastructureFolder, name2));
          }
    	}
    	catch (Exception ex)
    	{
