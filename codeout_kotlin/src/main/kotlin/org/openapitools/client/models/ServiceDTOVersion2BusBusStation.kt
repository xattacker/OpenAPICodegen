...
import org.openapitools.client.models.ServiceDTOVersion2BusStationStop

/**
 * 站牌/位資料型別
 * @param stationUID 站位唯一識別代碼，規則為 {業管機關簡碼} + {StationID}，其中 {業管機關簡碼} 可於Authority API中的AuthorityCode欄位查詢
 * @param stationID 站位代碼
 * @param stationName 
 * @param stationPosition 
 * @param stationAddress 站位地址
 * @param stops 站牌與所行經此站牌之路線列表(資料會對路線展開，因此可能會有重複的站牌資料)
 * @param updateTime 資料更新日期時間(ISO8601格式:yyyy-MM-ddTHH:mm:sszzz)
 * @param versionID 資料版本編號
 */
data class ServiceDTOVersion2BusBusStation (
    /* 站位唯一識別代碼，規則為 {業管機關簡碼} + {StationID}，其中 {業管機關簡碼} 可於Authority API中的AuthorityCode欄位查詢 */
    val stationUID: kotlin.String,
    /* 站位代碼 */
    ...
    /* 站位地址 */
    val stationAddress: kotlin.String? = null,
    /* 站牌與所行經此站牌之路線列表(資料會對路線展開，因此可能會有重複的站牌資料) */
    val stops: kotlin.Array<ServiceDTOVersion2BusStationStop>? = null
) {
