...
/**
 * 路段動態資訊
 * @param routeID 路段編碼(營運業者代碼+路段原編碼)
 * @param sourceID 營運業者代碼
 * @param level 級別(以 1,2,3,4,…方式描述服務水準級別，另外 -99 代表資料不足)
 * @param value 門檻指標數值，例如對照之門檻指標為路段平均 速率，value=10 (單位：kph)
 * @param travelTime 路段旅行時間，單位：秒
 * @param datacollectTime 資料蒐集時間
 ...
 */
data class ServiceDTOVersion2RoadRoadLive (
    /* 路段編碼(營運業者代碼+路段原編碼) */
    val routeID: kotlin.String,
    /* 營運業者代碼 */
    val sourceID: kotlin.String,
    /* 級別(以 1,2,3,4,…方式描述服務水準級別，另外 -99 代表資料不足) */
    val level: kotlin.Int? = null,
    /* 門檻指標數值，例如對照之門檻指標為路段平均 速率，value=10 (單位：kph) */
    val value: kotlin.Int? = null,
    /* 路段旅行時間，單位：秒 */
    val travelTime: kotlin.Float? = null,
    /* 資料蒐集時間 */
