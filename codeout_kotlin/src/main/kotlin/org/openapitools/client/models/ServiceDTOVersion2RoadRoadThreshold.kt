 ...
 * @param level 級別(以 1,2,3,4,…方式描述服務水準級別，另外-99 代表資料不足) levelname
 * @param levelName 級別文字描述
 * @param index 門檻指標說明(文字說明使用的門檻指標是速率、佔有率還是某綜合指標，並包括位置及時間等情境資訊)
 * @param topValue 門檻值上限(若門檻值無上限則免填)
 * @param lowValue 門檻值下限(若門檻值無下限則填寫 0)
 * @param colorR R 值(RGB 三原色代表路段呈現顏色，最高值為 255，最小值為 0)
 * @param colorG G 值(RGB 三原色代表路段呈現顏色，最高值為 255，最小值為 0)
 * @param colorB B 值(RGB 三原色代表路段呈現顏色，最高值為 255，最小值為 0)
 ...
 */
data class ServiceDTOVersion2RoadRoadThreshold (
    /* 營運業者代碼 */
    val sourceID: kotlin.String,
    /* 級別(以 1,2,3,4,…方式描述服務水準級別，另外-99 代表資料不足) levelname */
    val level: kotlin.String? = null,
    /* 級別文字描述 */
    val levelName: kotlin.String? = null,
    /* 門檻指標說明(文字說明使用的門檻指標是速率、佔有率還是某綜合指標，並包括位置及時間等情境資訊) */
    val index: kotlin.String? = null,
    /* 門檻值上限(若門檻值無上限則免填) */
    val topValue: kotlin.String? = null,
    /* 門檻值下限(若門檻值無下限則填寫 0) */
    val lowValue: kotlin.String? = null,
    /* R 值(RGB 三原色代表路段呈現顏色，最高值為 255，最小值為 0) */
    val colorR: kotlin.String? = null,
    /* G 值(RGB 三原色代表路段呈現顏色，最高值為 255，最小值為 0) */
    val colorG: kotlin.String? = null,
    /* B 值(RGB 三原色代表路段呈現顏色，最高值為 255，最小值為 0) */
    val colorB: kotlin.String? = null,
