...
/**
 * AVI 動態資訊
 * @param avIPairID AVI 配對代碼(營運業者代碼+配對原編號)
 * @param sourceID 營運業者代碼
 * @param startAVIStatus 配對起始點設備狀態 (正常為 0，通訊異常為 1，停 用或施工中為 2，設備故障為 3)
 * @param endAVIStatus 配對結束點設備狀態(正常為 0，通訊異常為 1，停用 或施工中為 2，設備故障為 3)\"
 * @param travelTime 旅行時間，單位：秒，另外-99 代表資料不足
 * @param datacollectTime 資料蒐集時間(時間資料格式為 24 小時制)
 ...
 */
data class ServiceDTOVersion2RoadTrafficAVIAVILive (
    /* AVI 配對代碼(營運業者代碼+配對原編號) */
    val avIPairID: kotlin.String,
    /* 營運業者代碼 */
    val sourceID: kotlin.String,
    /* 配對起始點設備狀態 (正常為 0，通訊異常為 1，停 用或施工中為 2，設備故障為 3) */
    val startAVIStatus: kotlin.String? = null,
    /* 配對結束點設備狀態(正常為 0，通訊異常為 1，停用 或施工中為 2，設備故障為 3)\" */
    val endAVIStatus: kotlin.String? = null,
    /* 旅行時間，單位：秒，另外-99 代表資料不足 */
    val travelTime: kotlin.String? = null,
    /* 資料蒐集時間(時間資料格式為 24 小時制) */
    val datacollectTime: kotlin.String? = null,
